...
export default function Ministries() {
  ...
  const ministriesList = [
    { 
      name: t.ministries.list.kids.name, 
      icon: <Smile size={32} />, 
      color: "text-blue-500", 
      bg: "bg-blue-50",
      details: {
        title: t.ministries.list.kids.details.title,
        paragraphs: t.ministries.list.kids.details.paragraphs,
        membersTitle: t.ministries.list.kids.details.membersTitle,
        membersCount: 3
      }
    },
    { 
      name: t.ministries.list.worship.name, 
      icon: <Music size={32} />, 
      color: "text-brand-gold", 
      bg: "bg-brand-gold/10",
      details: {
        title: t.ministries.list.worship.details.title,
        paragraphs: t.ministries.list.worship.details.paragraphs,
        membersTitle: t.ministries.list.worship.details.membersTitle,
        membersCount: 3
      }
    },
    { 
      name: t.ministries.list.communication.name, 
      icon: (
      ...
      bg: "bg-indigo-50",
      details: {
        title: t.ministries.list.communication.details.title,
        paragraphs: t.ministries.list.communication.details.paragraphs,
        membersTitle: t.ministries.list.communication.details.membersTitle,
        membersCount: 3
      }
    },
  ];
            ...
            >
              ...
              <div className="p-8 md:p-10">
                ...
                {selectedMinistry.details ? (
                          ...
                          {Array.from({ length: selectedMinistry.details.membersCount || 0 }).map((_, idx) => (
                            <div key={idx} className="flex flex-col items-center">
                              <div className="w-full aspect-[3/4] bg-gray-100 rounded-2xl mb-4 shadow-inner flex flex-col items-center justify-center overflow-hidden border border-gray-200">
                                <span className="text-gray-400 text-sm font-medium">{t.ministries.memberPhoto}</span>
                              </div>
                              <p className="font-semibold text-gray-800 text-center">{t.ministries.memberName}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <p className="text-gray-500 text-lg">{t.ministries.moreInfo}</p>
                )}
              </div>
            </motion.div>
          </div>
